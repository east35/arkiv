...
export function NotesList({ notes, onCreate, onUpdate, onDelete }: NotesListProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {notes.length === 0 && (
        <p className="text-sm text-muted-foreground">No notes yet. Add your thoughts below.</p>
      )}

      {notes.map((note) => (
        <div key={note.id} className="bg-card border rounded-lg p-3 space-y-2">
          {editingId === note.id ? (
            <>
              <Textarea
                value={editContent}
